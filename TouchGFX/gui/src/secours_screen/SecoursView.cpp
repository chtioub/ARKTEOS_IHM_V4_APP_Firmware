...
SecoursView::SecoursView()
{
	...
	// Titre
	Unicode::snprintf(textAreaBuffer_Titre, 40, touchgfx::TypedText(T_TEXT_CONFIGURATION_SECOURS_CENTRE_DEFAUT).getText());
	barre_titre.titre(textAreaBuffer_Titre);
}

void SecoursView::setupScreen()
{
    SecoursViewBase::setupScreen();
}

void SecoursView::tearDownScreen()
{
    SecoursViewBase::tearDownScreen();
}

void SecoursView::bouton_secours_ecs()
{
	eOuiNon = OUI_NON_MODE_SECOURS_ECS;
	application().gotoPage_oui_nonScreenNoTransition();
}

void SecoursView::bouton_secours_pac()
{
	eOuiNon = OUI_NON_MODE_SECOURS;
	application().gotoPage_oui_nonScreenNoTransition();
}
...
void SecoursView::changeConfig(S_CONFIG_IHM *sConfig_IHM)
{
...
void SecoursView::changeErreur(uint16_t u16Erreur)
{
...
void SecoursView::changeDate(S_DATE *sDate)
{

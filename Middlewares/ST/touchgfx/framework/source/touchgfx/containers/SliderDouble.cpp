...
#include <touchgfx/containers/SliderDouble.hpp>
      ...
      newValueCallback(0)
{
    ...
    // Default value range
    SliderDouble::updateIndicatorPosition(indicatorMinPosition);
}

void SliderDouble::setBitmaps(const Bitmap& sliderBackground, const Bitmap& sliderBackgroundSelected, const Bitmap& indicatorBitmap)
{
...
void SliderDouble::setBitmaps(const BitmapId sliderBackground, const BitmapId sliderBackgroundSelected, const BitmapId indicatorBitmap)
{
    setBitmaps(Bitmap(sliderBackground), Bitmap(sliderBackgroundSelected), Bitmap(indicatorBitmap));
}

void SliderDouble::setupHorizontalSlider(int16_t backgroundX, int16_t backgroundY, int16_t indicatorY, int16_t indicatorMinX, int16_t indicatorMaxX)
{
...
void SliderDouble::setupVerticalSlider(int16_t backgroundX, int16_t backgroundY, int16_t indicatorX, int16_t indicatorMinY, int16_t indicatorMaxY)
{
...
void SliderDouble::setValue(int16_t value)
{
    updateIndicatorPosition(valueToPosition(value));
}

void SliderDouble::handleClickEvent(const ClickEvent& event)
{
...
void SliderDouble::handleDragEvent(const DragEvent& event)
{
    updateIndicatorPosition((sliderOrientation == HORIZONTAL ? event.getNewX() : event.getNewY()) - getIndicatorRadius());
}

int16_t SliderDouble::valueToPosition(int value) const
{
...
int SliderDouble::positionToValue(int16_t position) const
{
...
void SliderDouble::updateIndicatorPosition(int16_t position)
{
...
int16_t SliderDouble::getIndicatorRadius() const
{
    return (sliderOrientation == HORIZONTAL ? indicator.getWidth() : indicator.getHeight()) / 2;
}

void SliderDouble::setValueRange(int16_t minValue, int16_t maxValue, int16_t newValue)
{
...
void SliderDouble::setValueRange(int16_t minValue, int16_t maxValue)
{
...
int SliderDouble::getIndicatorPositionRangeSize() const
{
    return indicatorMaxPosition - indicatorMinPosition;
}

int SliderDouble::getValueRangeSize() const
{
